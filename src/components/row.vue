<template>
  <div
    ...
    :class="isOpen && !isEditing ? 'bg-slate-800/40 dark:bg-white/[0.03] rounded-2xl border border-slate-700/40 dark:border-white/[0.05]' : 'rounded-xl'"
  >
    <!-- ── View Mode ─────────────────────────────────────────── -->
    <template v-if="!isEditing">
      <div class="flex items-center gap-3 px-2 py-3">
        ...
        <!-- Accent bar -->
        <div class="flex-shrink-0 w-[3px] h-9 rounded-full bg-indigo-500 dark:bg-indigo-400" />
        ...
        <div class="flex-grow min-w-0">
          ...
          <div v-if="projectName" class="flex items-center gap-1.5">
            <svg xmlns="http://www.w3.org/2000/svg" class="w-3 h-3 text-indigo-400/60 shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
              <rect x="2" y="7" width="20" height="14" rx="2" ry="2"/>
              <path d="M16 21V5a2 2 0 0 0-2-2h-4a2 2 0 0 0-2 2v16"/>
            </svg>
            <span class="text-[11px] text-slate-400 dark:text-slate-500 font-medium truncate uppercase tracking-wide">{{ projectName }}</span>
          </div>
        </div>

        <!-- Time -->
        <div class="flex-shrink-0 flex items-center gap-1.5 font-mono font-bold text-sm text-slate-200 dark:text-slate-200">
        ...
        <div class="flex items-center gap-0.5 shrink-0">
          ...
          >
            <svg v-if="isCopied" xmlns="http://www.w3.org/2000/svg" class="w-3.5 h-3.5 text-emerald-400" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round">
          ...
          <button
            ...
            :class="[
              syncSuccess ? 'text-emerald-400 hover:bg-white/5' :
              syncError   ? 'text-rose-400 hover:bg-white/5'   :
          ...
          >
            <span v-if="isSyncing" class="block w-3.5 h-3.5 border-2 border-current border-t-transparent rounded-full animate-spin"/>
            <svg v-else-if="syncSuccess" xmlns="http://www.w3.org/2000/svg" class="w-3.5 h-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round">
              <polyline points="20 6 9 17 4 12"/>
            </svg>
            <svg v-else-if="syncError" xmlns="http://www.w3.org/2000/svg" class="w-3.5 h-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
          ...
          >
            <svg v-if="isOpen" xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round">
      ...
      >
        <div class="px-6 pb-3">
          <div class="p-3 bg-black/30 dark:bg-black/40 rounded-xl border border-white/[0.04]">
            <p class="text-xs text-slate-400 leading-relaxed italic">
              <span class="not-italic font-bold text-indigo-400/40 mr-2">—</span>{{ activity.name }}
            </p>
          </div>
        </div>
      </div>
    ...
    <template v-else>
      <div class="flex flex-col gap-2 w-full p-3">
        <div class="flex items-center gap-2">
          ...
          />
          <div class="flex items-center bg-slate-900 border border-slate-700 rounded-lg overflow-hidden shrink-0 focus-within:ring-2 focus-within:ring-cyan-500">
            <input type="number" v-model="editHours"   min="0"       class="w-10 px-1 py-1.5 text-sm text-center bg-transparent outline-none text-slate-100"/>
            <span class="text-slate-600">:</span>
            <input type="number" v-model="editMinutes" min="0" max="59" class="w-10 px-1 py-1.5 text-sm text-center bg-transparent outline-none text-slate-100"/>
          </div>
        </div>
        <div class="flex items-center gap-2">
          <select
          ...
          <div class="flex gap-1 shrink-0">
            <button @click="saveEdit" :disabled="!isValid" class="cursor-pointer p-1.5 text-emerald-400 hover:bg-emerald-500/10 rounded-lg disabled:opacity-50 disabled:cursor-not-allowed">
              <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M20 6L9 17l-5-5"/></svg>
            </button>
            <button @click="cancelEdit" class="cursor-pointer p-1.5 text-slate-500 hover:text-slate-300 hover:bg-white/5 rounded-lg">
...
// ── Computed ───────────────────────────────────────────────────
const displayTitle = computed(() => props.activity.ticketTitle || props.activity.name);

const projectName = computed(() => {
  if (!props.activity.projectId || !props.projects) return "";
  return props.projects.find((p) => p.id === props.activity.projectId)?.name ?? "";
});
